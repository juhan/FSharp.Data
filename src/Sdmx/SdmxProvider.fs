...
type public SmdxProvider(cfg:TypeProviderConfig) as this =
    ...
    let ns = "FSharp.Data"
    let wsEntryPoint = "http://api.worldbank.org" //"https://api.worldbank.org/v2/sdmx/rest"
    let cacheDuration = TimeSpan.FromDays 30.0
    let restCache = createInternetFileCache "SdmxSchema" cacheDuration
    
    let createTypesForSources(sources, sdmxTypeName, asynchronous) = 
    
        ProviderHelpers.getOrCreateProvidedType cfg this sdmxTypeName <| fun () ->

        let connection = ServiceConnection(restCache, wsEntryPoint, sources)
        ...
        let sdmxDataServiceType =
            ...
            t.AddMembersDelayed (fun () -> 
                [ yield ProvidedProperty("Countries", countriesType,  getterCode = (fun (Singleton arg) -> <@@ ((%%arg : SdmxData) :> ISdmxData).GetCountries() @@>))
                  yield ProvidedProperty("Regions", regionsType,  getterCode = (fun (Singleton arg) -> <@@ ((%%arg : SdmxData) :> ISdmxData).GetRegions() @@>))
                  yield ProvidedProperty("Topics", topicsType,  getterCode = (fun (Singleton arg) -> <@@ ((%%arg : SdmxData) :> ISdmxData).GetTopics() @@>)) ])
            serviceTypesType.AddMember t
            t

        resTy.AddMembersDelayed (fun () -> 
            [ let urlVal = wsEntryPoint
              let sourcesVal = sources |> String.concat ";"
              yield ProvidedMethod ("GetDataContext", [], sdmxDataServiceType, isStatic=true,
                                       invokeCode = (fun _ -> <@@ SdmxData(urlVal, sourcesVal) @@>)) 
            ])
    ...
    let sdmxType = createTypesForSources(defaultSources, "SdmxData", false)
    do sdmxType.AddXmlDoc "<summary>Typed representation of WorldBank data. See http://www.worldbank.org for terms and conditions.</summary>"

    let paramSdmxType = 
        let t = ProvidedTypeDefinition(asm, ns, "SdmxDataProvider", None, hideObjectMethods = true, nonNullable = true)
    ...
    do this.AddNamespace(ns, [ sdmxType; paramSdmxType ])
